...
**Versi Dokumen:** 2.0  
**Tanggal:** June 2025  
**Status:** SIAP UNTUK IMPLEMENTASI IT  
**Pemilik:** Departemen Pengadaan Umum dan Jasa  
...
5. [Spesifikasi API](#api-specifications)
...
12. [Panduan Deployment](#deployment-guide)
...
- ✅ Vendor self-service sjph submission (any document format)
...
**External Users:**
- **Vendors/Rekanan** (registered suppliers; submit sjph, negotiate, track orders)
...
**FR2: SJPH Submission (Vendor)**
- Upload sjph in any format (PDF, Word, image, scanned)
...
- Bid tabulation: <5 minutes for 50 sjph
...
| **File Storage** | AWS S3 OR Local FS | - | SJPH documents |
...
CREATE TABLE payments (
  ...
  grn_matched BOOLEAN DEFAULT false, -- Berita Acara Penerimaan Barang
...
CREATE TABLE audit_logs (
  ...
  entity_type VARCHAR(50), -- 'tender', 'sjph', 'po', 'payment'
  entity_id INT,
...
CREATE INDEX idx_quotes_tender_vendor ON sjph(tender_id, vendor_id);
...
**POST /sjph**
```
Submit sjph for tender (Vendor only)

Form Data (multipart/form-data):
- tender_id: 15
- document: <binary file> (PDF/Word/Image)
- additional_notes: "Optional notes about the sjph"
...
Response (201):
{
  ...
  "data": {
    ...
    "tender_id": 15,
    "vendor_id": 5,
    "document_url": "s3://bucket/sjph/TND-2025-0015-V5-R1.pdf",
...
**GET /sjph/{quote_id}**
```
Get sjph details (with extracted data)
...
**GET /tenders/{tender_id}/sjph**
```
List all sjph for a tender (Internal only)
...
Response (200):
{
  ...
  "data": {
    ...
    "sjph": [
      ...
      // ... more sjph ordered by score
    ]
  }
}
```

**POST /sjph/{quote_id}/revise**
```
Vendor revise sjph (negotiate)
...
**POST /sjph/{quote_id}/manual-review**
```
...
```
Trigger AI scoring for all sjph in tender
...
const Anthropic = require("@anthropic-ai/sdk"); // LLM Integration via Anthropic SDK
...
async function extractQuoteData(documentPath, documentType) {
  ...
  const prompt = `
    ...
    A vendor has submitted a sjph/proposal. Your task is to extract the following structured data from the document:
...
#### 1. SJPH Tabulation Table (Internal)
```
...
│  Trigger: Goods received (BAPB-2025-001)
...
│ Status: 5 sjph submitted              │
│ Action: [View SJPH] [Score All]       │
└─────────────────────────────────────────┘
...
│ Status: 8 sjph | Scoring complete    │
...
└──────────────────────────────────────────────────────────────────┘
  ...
  │ ├─ Generate BAPB (Berita Acara Penerimaan Barang)   │
...
- [ ] SJPH Submission (Vendor)
  - [ ] Upload sjph document
  - [ ] Document storage (S3/local)
  - [ ] SJPH status tracking
  - [ ] Revision capability
...
- [ ] PO Generation
  - [ ] Auto-generate from selected sjph
...
**Example: SJPH Upload to Scoring**
```javascript
// test/integration/sjph-workflow.test.js

describe('SJPH Submission to Scoring Workflow', () => {
  
  test('should extract, validate, and score a sjph', async () => {
    // 1. Upload sjph
    const uploadResponse = await request(app)
      .post('/api/v1/sjph')
      .attach('document', './test-fixtures/sjph.pdf')
...
describe('Performance Tests', () => {
  ...
  test('should handle 50 simultaneous sjph uploads', async () => {
...
**Q: AI extraction failing for handwritten sjph**
...
- Focus on tender creation & sjph evaluation
...
## PERBAIKAN ANALISIS CELAH (PENINGKATAN ENTERPRISE)

Berdasarkan analisis celah (gap analysis) untuk standar *Enterprise/Healthcare*, sistem ini diperkuat dengan empat (4) pilar arsitektur tambahan:

### 1. Vendor E-KYB (Know Your Business) Verification
Sistem tidak otomatis menerima pendaftaran vendor. Alur baru:
1. Vendor mengunggah dokumen legalitas (NIB, SIUP, Akta Perusahaan).
2. Status vendor menjadi `pending_verification`.
3. Admin Pengadaan meninjau dokumen secara manual melalui Dashboard E-KYB.
4. Admin menekan tombol "Approve" untuk mengaktifkan vendor.
*(Catatan: Modul disiapkan secara asinkron (API placeholder) agar kelak dapat disambungkan otomatis ke API OSS / Ditjen Pajak).*

### 2. HitL (Human-in-the-Loop) AI Governance
Model Gemini digunakan untuk mengekstrak aturan SOP menjadi JSON. Namun, untuk mencegah halusinasi yang memblokir operasional:
1. Hasil ekstraksi AI akan disimpan dengan status `draft_rule`.
2. *Compliance Officer* / Manager Pengadaan memiliki antarmuka (UI) untuk me-review draf aturan (berdampingan dengan teks SOP asli).
3. Aturan baru akan memblokir (*enforce*) transaksi HANYA setelah Officer menekan tombol "Activate Rule".

### 3. SIMRS Webhook (Event-Driven Architecture)
Untuk menjembatani Modul Pengadaan dengan sistem rumah sakit (Sistem Informasi Manajemen Rumah Sakit / SIMRS):
1. Setiap kali BAPB (*Berita Acara Penerimaan Barang*) disetujui (Goods Received).
2. Sistem akan memancarkan (emit) sebuah *Webhook Event* (`bapb.approved`).
3. SIMRS menangkap webhook ini dan secara otomatis membuat *draft* aset di modul Inventori/Kalibrasi Alat Kesehatan.

### 4. Audit Trail Immutability (WORM Storage)
Untuk mencegah manipulasi log (bahkan oleh DBA):
1. Tabel `audit_logs` diimplementasikan dengan sistem *cryptographic hashing*.
2. Setiap baris baru mencatat `previous_hash` (dari *record* sebelumnya) dan `current_hash` (hash dari payload data baris tersebut + `previous_hash`).
3. Sistem mendeteksi *tampering* jika hash rusak, memastikan validitas di mata auditor eksternal.
...
## INTEGRASI #1: UNIT LUAR (Branch Units / Clinical Departments)
...
- Confirm receipt and quality (BAPB creation)
...
**3. Unit Confirm Receipt & Create BAPB**
```
POST /api/v1/bapb
{
  "po_id": 100,
...
│    BAPB: BAPB-2025-001 | QC: PASSED | Ready for use              │
│    [VIEW BAPB] [VIEW QC REPORT]                                  │
└──────────────────────────────────────────────────────────────────┘
...
## INTEGRASI #2: KEUANGAN (Finance/Accounting System)
...
System Auto-Validates:
  ...
  ✅ Quantities match
  ✅ Dates logical (Invoice after BAPB, BAPB after PO)
...
Response (if mismatch):
{
  ...
  "discrepancy_details": {
    ...
    "reason": "Invoice higher than BAPB"
  },
...
## INTEGRASI #3: BOD (Direksi PT KMU)

### Use Case
BOD members (President, GM Pengembangan Usaha, Marketing dan Pengadaan, Direktur Keuangan, SDM dan Umum) need:
...
│ Cost Savings (vs sjph): 156,000,000 IDR (3.2%)               │
└──────────────────────────────────────────────────────────────────┘
...
## INTEGRASI #4: PAYMENT ALERT SYSTEM
...
## INTEGRASI #5: SPI (SISTEM PENGENDALIAN INTERNAL / Internal Control System)
...
│ • 2025-06-21 09:00 | Unit (Warehouse) | Received goods BAPB-001 │
│                                                                  │
...
const SPI_RULES = {
  ...
  documentation_completeness: {
    ...
    required_docs: ["invoice", "bapb", "qc_report"],
    check: (po) => {
...
4. VENDORS SUBMIT QUOTES
   ...
   └─ Alert: Procurement team "3 new sjph received"
...
11. GOODS RECEIVED
    ...
    └─ Alert: Procurement "BAPB received"
...
### New Integrations Added to Cetak Biru

| Integration | API Calls | Alerts | GL Posts | Audit | 
|-------------|-----------|--------|----------|-------|
| Unit Luar | 4 (reqn, track, BAPB, QC) | 5 types | Yes | Full trail |
