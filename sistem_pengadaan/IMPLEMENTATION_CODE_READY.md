...
CREATE TABLE validation_rules (
  ...
  status VARCHAR(20) DEFAULT 'draft',     -- draft (HitL review needed), active, archived
...
CREATE TABLE compliance_events (
  ...
  entity_type VARCHAR(50),                -- purchase_order, tender, sjph
  entity_id INT,
...
## BAGIAN ##: BACKEND API IMPLEMENTATION
...
class ProcedureValidator:
    ...
    def __init__(self, db_connection_string: str, pinecone_key: str, redis_url: str = "redis://localhost:6379"):
    ...
    async def validate_action(
        ...
        entity_type: str,           # purchase_order, tender, sjph
        entity_id: int,
        context: Dict
    ) -> ValidationResult:
        """
    ...
    def _mask_sensitive_data(self, text: str) -> str:
        """DLP: Mask NPWP, bank accounts, and specific names before sending to LLM"""
        # Basic regex masking (in production use Microsoft Presidio or similar)
        text = re.sub(r'\b\d{15}\b', '[MASKED_NPWP]', text)
        text = re.sub(r'\b\d{10,16}\b', '[MASKED_ACCOUNT]', text)
        return text
    ...
    async def _generate_guidance(
        ...
        """Use Claude to generate helpful guidance (with Cache & DLP)"""
        ...
        # 1. Apply DLP Data Masking
        safe_prompt = self._mask_sensitive_data(prompt)
        
        # 2. Semantic Caching Check
        cache_key = hashlib.md5(safe_prompt.encode('utf-8')).hexdigest()
        cached_response = self.cache.get(f"guidance:{cache_key}")
        if cached_response:
            return cached_response.decode('utf-8')
            
        # 3. Model Routing (Use faster/cheaper model for simple violations)
        # We use Claude 3.5 Haiku instead of Opus for real-time validation guidance
        ...
        self.cache.setex(f"guidance:{cache_key}", 86400, guidance)
...
## BAGIAN ##: FRONTEND IMPLEMENTATION
...
## BAGIAN ##: DATABASE INITIALIZATION SCRIPT
...
('RULE-004', '3-Way Match Requirement', 'document', 'SOP-004', '{"required":["po","bapb","invoice"]}', 'high', true);
...
Response:
{
  ...
  "response": "Instruksi langkah demi langkah untuk Kepala Bagian Keuangan dalam melakukan persetujuan PO.",
